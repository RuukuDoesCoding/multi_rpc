...
def handle_valorant():
    if not is_running("VALORANT.exe"):
        return False
...
def main_loop():
    global running
    while running:
        lol_running = is_running("LeagueClient.exe")
        valo_running = is_running("VALORANT.exe")
        updated = False

        if lol_running:
            updated = handle_lol() or updated
        else:
            try:
                rpc_lol.clear()
            except Exception as e:
                logging.info(f"Failed to clear LoL RPC: {e}")

        if valo_running:
            updated = handle_valorant() or updated
        else:
            try:
                rpc_valo.clear()
            except Exception as e:
                logging.info(f"Failed to clear Valorant RPC: {e}")
